...
def augmentation(concat):
    ...
    if "y" in concat.columns:
        ...
        y = None
    X = pd.DataFrame(poly.fit_transform(
        X), columns=poly.get_feature_names(), index=concat.index)
    return X, y
...
predictions_surprize_final = Surprize.main(
    input_.copy(), format_.copy(), cache_name="final")

# training the ALS model
predictions_als_final = ALS.main(
    input_.copy(), format_.copy(), cache_name="final")

# training the Kmeans variant model
predictions_kmeans_final = Kmeans.main(
    input_.copy(), format_.copy(), rounded=False)
...
for i in range(concat_aug_final.shape[1]-1):
